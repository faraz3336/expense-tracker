...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    @Query(value = "SELECT COUNT(*) FROM transactions t JOIN users u ON t.user_id = u.id " +
            "WHERE u.id = :userId AND EXTRACT(MONTH FROM t.date) = :month AND EXTRACT(YEAR FROM t.date) = :year", nativeQuery = true)
    ...
    @Query(value = "SELECT " +
            ...
            "JOIN category c on t.category_id = c.category_id " +
            "JOIN transaction_type tt on c.transaction_type_id = tt.transaction_type_id " +
            "WHERE u.email = :email and t.date >= CURRENT_DATE - INTERVAL '5 months' " +
            "GROUP BY EXTRACT(YEAR FROM t.date), EXTRACT(MONTH FROM t.date)", nativeQuery = true)
